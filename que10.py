...
# Iski output kuch aisi aayegi: * Yahan pehle woh for small_list in big_list wala loop chala ke uski pehli iteration mein pahunchta hai.
...
def max_marks(marks):
    for i in range (len(marks)):
        for k in range(len(marks[i])-1):
            if marks[i][k]>marks[i][k+1]:
                temp=marks[i][k]
                marks[i][k]=marks[i][k+1]
                marks[i][k+1]=temp
        print(marks[i][k+1],"is the maximum marks scored by the student")
marks = [[45, 21, 42, 63], [12, 42, 42, 53], [42, 90, 78, 13], [94, 89, 78, 76], [87, 55, 98, 99]]
max_marks(marks)
...
# Ek max_marks naam ka function banao 
# jo ki ek aisi list le aur har students ke maximum marks print kare. 
